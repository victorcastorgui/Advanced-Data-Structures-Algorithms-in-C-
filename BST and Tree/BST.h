...
private:
    ...
    static Node *_deep_copy(const Node *p) {
        ...
        Node *cpyRgt = _deep_copy(p->_right);
        Node *cpyLft = _deep_copy(p->_left);
        Node *temp = new Node(p->_data, cpyLft, cpyRgt);
        return temp;
    }
...
public:
    ...
    virtual string to_string() const {return _to_string(_root);}
